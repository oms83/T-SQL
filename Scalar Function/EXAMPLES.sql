...
Alter Function GetLastGradeOfSubjet(@studnetID int, @subjectTilte varchar(100))
returns int
as
begin
	declare @LastGrade int;

	Select Top 1 @LastGrade = T.grade from Students2 S
	inner join TakeExams T on T.studnetID = S.studentID
	inner join Sujbects U on U.subjectID = T.subjectID
...
select dbo.GetLastGradeOfSubjet(1, 'IQ')
-- 100
